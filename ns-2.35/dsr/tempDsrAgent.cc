...
void DSRAgent::addToNeighbourLoc(ID id, double x, double y, int status, Time t) {
	...
	if(calcDistance(myX, x, myY, y) > 250.0) {
		printf("Distance ambiguity %u %u at time %lf\n", id.addr, net_id.addr, Scheduler::instance().clock());
		return;
	}
	if(status == 1) {
  ...
  // p.pkt is freed or handed off
{
  ...
  srh->rtreq_seq() = route_request_num++;
	srh -> route_req_path_timeout() = 500.0;
  ...
  *** do not change or free packet *** */
{
  ...
  if (srh->route_reply())
    {
	timeout = timeout < srh -> route_rep_path_timeout() ? timeout: srh -> route_rep_path_timeout();
